...
func_definition : type_specifier ID LPAREN parameter_list RPAREN {id++; symtab.scopeid = id; symtab.enterScope(bucket,id); scopeParameters();} compound_statement
				{
				...
				| type_specifier ID LPAREN RPAREN {id++;symtab.scopeid = id; symtab.enterScope(bucket,id); scopeParameters();} compound_statement
				{
					//symtab.PrintAllScope();
...
declaration_list : declaration_list COMMA ID
                 ...
                 | declaration_list COMMA ID LTHIRD CONST_INT RTHIRD
                 {
                 	...
                 	size = stoi($5->getName());
					variableListInitialize.push_back({var_name + int2s(symtab.getCurrentScopeID()), bracket_material});
...
expression : logic_expression
			...
			| variable ASSIGNOP logic_expression
			{
				...
				if($1->index != -1)
				{
					asmcodes+= "\nMOV "+temp_name+ "+" + int2s($1->index)+ "*2, AX"+ "\n";
				}
				if($1->index == -1)
				{
					asmcodes+= "\nMOV "+ temp_name + ", AX"+"\n";
...
rel_expression : simple_expression
				...
				| simple_expression RELOP simple_expression
				{
					...
					asmcodes += "\nMOV AX, "+$1->assemblyName+"\n";
					asmcodes += "\nCMP AX, "+$3->assemblyName+"\n";

					if($2->getName() == "<") { asmcodes += "\nJL "+labelTrue+"\n"; }
					else if($2->getName() == "<=") { asmcodes += "\nJLE "+labelTrue+"\n"; }
					else if($2->getName() == ">") { asmcodes += "\nJG "+labelTrue+"\n"; }
					else if($2->getName() == ">=") { asmcodes += "\nJGE "+labelTrue+"\n"; }
					else if($2->getName() == "==") { asmcodes += "\nJE "+labelTrue+"\n"; }
					else if($2->getName() == "!=") { asmcodes += "\nJNE "+labelTrue+"\n"; }
				  ...
				  | simple_expression ADDOP term
				  {
					...
					//fprintf(errorout, " $3 -- %s",$3->getVarType().c_str() ); 
					 ...
					 //assembly##########################
						...
						if($2->getName()=="+"){
				            asmcodes+=("\nMOV AX, "+$1->assemblyName+"\n");
				            asmcodes+=("ADD AX, "+$3->assemblyName+"\n");
				            asmcodes+=("MOV "+temp_name+", AX\n");
			            }
			
			            if($2->getName()=="-"){
				             asmcodes+=("\nMOV AX, "+$1->assemblyName+"\n");
				             asmcodes+=("SUB AX, "+$3->assemblyName+"\n");
			  	             asmcodes+=("MOV "+temp_name+", AX\n");
			            }
		...
		string b;
		 ...
		 //assembly#######################
			...
			{	
				asmcodes += "\XOR DX, DX\n";
				asmcodes += "\nDIV BX\n";
				asmcodes += "\nMOV "+temp_name+", DX\n"; // karon vagsesh dx er mddhe joma hoy
			}
